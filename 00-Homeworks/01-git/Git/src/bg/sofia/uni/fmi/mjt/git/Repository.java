...
public class Repository {
    ...
    public Result add(String... files) {
        ...
        for (String file : files) {
            if ((branches.get(head) != null && getFiles().contains(file))
                    || stagedFilesToAdd.contains(file))
            {
    ...
    public Result commit(String message) {
        if (stagedFilesToAdd.isEmpty() && stagedFilesToRemove.isEmpty()) {
            ...
            branches.replace(head, newCommit);

            int filesChanged = stagedFilesToRemove.size() + stagedFilesToAdd.size();

            Result result = new Result(filesChanged + " files changed", true);

            getFiles().removeAll(stagedFilesToRemove);
            getFiles().addAll(stagedFilesToAdd);
    ...
    public Result remove(String... files) {
        ...
        for (String file : files) {
            if (stagedFilesToRemove.contains(file) || (!getFiles().contains(file)))
            {
    ...
    //returns the files from the commit
    public Set<String> getFiles() {
        return getHead() == null ? new HashSet<>() : getHead().getFiles();
    }
